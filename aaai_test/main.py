...
class QF_NRA:
        ...
        for folder in folder_list:
            ...
            if os.path.isdir(new_path):
                ...
                with open(new_path) as f:
                    ...
                    self.records.loc[len(self.records.index)] = [
                        new_path, category, vars, assertions, status, '0', '0']
                ...
                try:
                    if self.data_path == 'result':
                        while f.readline():
                            ...
                            self.records.loc[self.records['filename'] == name, [
    ...
    def show(self):
        ...
        for folder in folder_list:
            ...
            solved.append(len(self.records[(self.records['category'] == folder) & (
                self.records['result'].str.contains('sat'))]))
            dump.append(len(self.records[(self.records['category'] == folder) & (
                self.records['result'] == ('dump'))]))
            timeout.append(len(self.records[(self.records['category'] == folder) & (
                self.records['result'] == ('timeout'))]))
        plt.figure(figsize=(18, 8))
        plt.bar(folder_list, solved, label="solved", color='green')
        plt.bar(folder_list, timeout, label="timeout",
                bottom=solved, color='orange')
    ...
    def delete(self, path):
        ...
        for folder in folder_list:
            ...
            if os.path.isdir(new_path):
                ...
                name = new_path[6:].replace(
                    'temp/', '').replace('_timeout', '')
